...
def loaddata():
    ...
    engine = create_engine(
        'mysql+pymysql://%(user)s:%(password)s@%(host)s:%(port)d/%(database)s?charset=utf8mb4' % db_info, encoding='utf-8')
    # 直接使用下一种形式也可以
...
if __name__ == '__main__':
    ...
    while True:
        entity = input("输入实体：")
        attribute = "类别"
        sql_e1 = "select * from nlpccQA where entity ='" + entity + "' order by length(entity) asc "  # asc表示升序
        #sql_e0 = "select * from nlpccQA where entity like '%" + entity +"%' order by length(entity) asc"
        # '%姚明'，搜的是xx姚明，'姚明%'，搜的是姚明xx
        sql_e0 = "select * from nlpccQA where match(entity) against('" + entity + "'in natural language mode)"
        start = datetime.now()
        result_e1 = upload_data(sql_e1)
        print(result_e1)
        print("精确查找共找到三元组{}个，精确查找用时{} sec".format(len(result_e1),(datetime.now()-start).seconds))
        print(sql_e0)
        start1 = datetime.now()
        result_e0 = upload_data(sql_e0)
        print(result_e0)
        print("模糊查找共找到三元组{}个，模糊查找用时{} sec".format(len(result_e0),(datetime.now()-start1).seconds))
