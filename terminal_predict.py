...
print('root path:{}'.format(FLAGS.output_dir))
...
def predict_outline():
    ...
    with graph.as_default():
        #print(id2label)
        nlpcc_test_data = pd.read_csv("./NLPCC2016KBQA/q_t_a_df_testing.csv")
        #nlpcc_test_data = nlpcc_test_data.head()
        correct = 0
        ...
        for row in nlpcc_test_data.index:
            ...
            #print('input the test sentence:%s' % question)
            ...
            if entity in result:
                ...
                print(i)
                with open("./NERdata/预测错误.txt", "a") as f:
                    f.writelines(question+"\t"+entity+"\t"+attribute+"\t"+answer+"\t"+','.join(result)+"\n")
                i+=1
            nlpcc_test_result.append(question+"\t"+entity+"\t"+attribute+"\t"+answer+"\t"+','.join(result))
            #print('time used: {} sec'.format((datetime.now() - start).seconds))

        with open("./NERdata/q_t_a_testing_predict.txt", "w") as f:
...
def predict_service(sentence):
    ...
    with graph.as_default():
        ...
        result = strage_combined_link_org_loc(sentence, pred_label_result[0])
        #print(result[0])
        print('问题中的实体识别用时: {} sec'.format((datetime.now() - start).seconds))
        return result[0]

if __name__ == "__main__":
    #predict_outline()
    #predict_online()
    predict_service("高等数学的出版社是哪个？")
